...
function createMessageElement(message) {
    ...
    // Nếu là tin nhắn từ AI, xử lý markdown
    if (message.role === 'assistant') {
    ...
    div.innerHTML = `
        ...
        ${message.role === 'assistant' ? `
            <div class="message-actions">
                ...
                <button class="action-btn feedback-btn" title="Đánh giá">
        ...
        if (copyBtn) {
            copyBtn.addEventListener('click', () => {
                navigator.clipboard.writeText(message.content)
                    .then(() => showSuccess('Đã sao chép vào clipboard'))
                    .catch(() => showError('Không thể sao chép'));
            });
        }
        
        const feedbackBtn = div.querySelector('.feedback-btn');
        if (feedbackBtn) {
            feedbackBtn.addEventListener('click', () => {
                // TODO: Implement feedback functionality
                showInfo('Tính năng đánh giá đang được phát triển');
            });
        }
    }
...
async function sendMessage(message) {
    try {
        // Lấy conversation_id hiện tại hoặc tạo mới
        ...
        if (!conversationId) {
            ...
            if (response.status === 'success') {
                ...
                throw new Error('Không thể tạo hội thoại mới');
            }
        }
        ...
        // Xử lý response và hiển thị tin nhắn từ AI
        if (response.status === 'success') {
