...
class SuggestionManager:
    ...
    async def generate_question_suggestions(
        ...
        """
        Tạo đề xuất câu hỏi dựa trên lịch sử hội thoại (bất đồng bộ)
        ...
        try:
            # Gọi LLM để sinh đề xuất bất đồng bộ
    ...
    def generate_question_suggestions_sync(
        self, conversation_history: str, num_suggestions: int = 3
    ) -> List[str]:
        """
        Tạo đề xuất câu hỏi dựa trên lịch sử hội thoại (đồng bộ - để tương thích ngược)
        ...
        try:
            # Gọi LLM để sinh đề xuất đồng bộ
            response = self.llm.invoke_sync(prompt)
            suggestions_text = response.content.strip()
    ...
    async def get_suggestions_from_latest_conversation(
        self, user_id: str, conversation_manager, num_suggestions: int = 3
    ) -> List[str]:
        """
        Lấy đề xuất câu hỏi dựa trên cuộc hội thoại gần đây nhất có tin nhắn (bất đồng bộ)
        ...
        try:
            ...
            return await self.generate_question_suggestions(
                formatted_conversation, num_suggestions
            )

        except Exception as e:
            print(f"Lỗi khi lấy đề xuất từ cuộc hội thoại gần đây: {str(e)}")
            return self._get_default_suggestions()[:num_suggestions]

    def get_suggestions_from_latest_conversation_sync(
        self, user_id: str, conversation_manager, num_suggestions: int = 3
    ) -> List[str]:
        """
        Lấy đề xuất câu hỏi dựa trên cuộc hội thoại gần đây nhất có tin nhắn (đồng bộ)
        ...
        try:
            ...
            # Tạo đề xuất từ lịch sử hội thoại đã định dạng
            return self.generate_question_suggestions_sync(
                formatted_conversation, num_suggestions
            )
